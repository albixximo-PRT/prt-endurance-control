...
<td className="border-b border-white/5 p-2 font-mono text-xs">
  ...
  {!releaseGrid.length ? (
    ...
    <>
      {/* TABELLA VISIBILE NEL PROGRAMMA */}
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="text-left text-zinc-400">
        ...
        <tbody>
          {releaseGrid.map((team, index) => (
            <tr key={`visible-${team.teamNumber}`}>
              <td className="border-b border-white/5 p-2 font-mono">
                {index + 1}
              </td>
      ...
      {/* LAYOUT NASCOSTO UTILIZZATO SOLO PER IL PNG */}
      <div
    ...
    />
          ...
          <div className="px-6 py-2">
            <table className="w-full table-fixed border-separate border-spacing-0 text-[13px]">
              <thead>
                <tr className="h-[30px] text-left text-[11px] font-black uppercase tracking-wider text-zinc-400">
                  <th className="w-[8%] border-b border-white/10 px-4 py-1">
                    {selectedLobby === 3 ? "Pos." : "Posizione"}
                  </th>

                  <th className="w-[15%] border-b border-white/10 px-4 py-1">
                    Team
                  </th>

                  <th className="w-[22%] border-b border-white/10 px-4 py-1">
                    Pilota
                  </th>
                  ...
                  <th className="w-[13%] border-b border-white/10 px-4 py-1 text-right">
                    {exportTimeLabel}
                  </th>
                </tr>
              </thead>
...
>
  ...
  Lap {team.pvcpCrashLap} • Pos {team.pvcpRacePosition}
                      ...
                      <td className="border-b border-white/[0.07] px-4 py-0 text-right">
                        <span
                          className={
                            isFirst
  ...
  <div className="flex translate-y-5 items-center gap-3">
    ...
    <img
      ...
      alt="xSamueLx"
      className="h-[50px] w-auto object-contain opacity-90"
    />
  </div>
